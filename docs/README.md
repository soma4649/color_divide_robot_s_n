...
4色(赤、緑、青、黃)のブロックをアームが投げて、色を仕分けることが出来ます。全部で６つのブロックと３つの箱を使い、ブロックは赤色が２つ、緑色が２つ、青色が１つ、黄色が１つです。あらかじめ決められている座標に置いてあるブロックを掴むため、定められた位置に置いてあるブロックしか掴むことは出来ません。また、投げる順番はプログラムで決められているため色の配置を変えてしまうと正しい箱に投げ入れることが出来ません。


## アピールポイント


・シュミレーション上に色付きの物体を表示させ、実機を使った環境に近づけた。

・実機での問題点として、アームがブロックに対して斜めに掴んでしまったり、ブロックが滑り落ちてしまうことがあったので、ブロックを少し斜めに置き、輪ゴムをつけることで解決した。

・投げるモーションを入れることで見た目的に面白くした。
